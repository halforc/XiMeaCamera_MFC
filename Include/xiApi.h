...
typedef enum
{
	...
	XI_BIN_SELECT_DEVICE_FPGA    =1, // FPGA Binning Engine on device side
	XI_BIN_SELECT_HOST_CPU       =2, //  CPU Binning Engine on Host side
...
typedef enum
{
	...
	XI_DEC_SELECT_DEVICE_FPGA    =1, // FPGA Decimation Engine on device side
	XI_DEC_SELECT_HOST_CPU       =2, //  CPU Decimation Engine on Host side
...
typedef enum
{
	...
	XI_TRG_OVERLAP_READ_OUT      =1, // Trigger is accepted only when sensor is ready to start next exposure with defined exposure time. Trigger is rejected when sensor is not ready for new exposure with defined exposure time.
...
typedef enum
{
	...
	XI_ACQ_TIMING_MODE_FRAME_RATE=1, // Selects a mode when sensor frame acquisition frequency is set to XI_PRM_FRAMERATE
	XI_ACQ_TIMING_MODE_FRAME_RATE_LIMIT=2, // Selects a mode when sensor frame acquisition frequency is limited by XI_PRM_FRAMERATE
...
typedef enum
{
	XI_TRANSPORT_DATA_TARGET_CPU_RAM=0, // Selects a CPU RAM as target for delivered data from the camera.
	XI_TRANSPORT_DATA_TARGET_GPU_RAM=1, // Selects a GPU RAM as target for delivered data from the camera.
...
typedef enum
{
	...
	XI_GPO_HIGH_IMPEDANCE        =12, // Hi impedance of output (if three state logic is used).
...
typedef enum
{
	XI_TEMP_IMAGE_SENSOR_DIE_RAW =0, // Not calibrated temperature of image sensor die (silicon) - e.g. sensor register value
	XI_TEMP_IMAGE_SENSOR_DIE     =1, // Calibrated temperature of image sensor die (silicon) - in degrees of Celsius
	XI_TEMP_SENSOR_BOARD         =2, // Sensor board temperature
	XI_TEMP_INTERFACE_BOARD      =3, // Interface board temperature
	XI_TEMP_FRONT_HOUSING        =4, // Front part of camera housing temperature
	XI_TEMP_REAR_HOUSING         =5, // Rear part of camera housing temperature
...
typedef enum
{
	XI_TEMP_CTRL_MODE_OFF        =0, // Temperature controlling is disabled (no fan or TEC (peltier) is enabled)
	XI_TEMP_CTRL_MODE_AUTO       =1, // Automated temperature controlling is enabled - based on selected thermomether and target temperature.
	XI_TEMP_CTRL_MODE_MANUAL     =2, // Manual controlling of temperature elements is enabled. Application can control the elements.
...
typedef enum
{
	XI_TEMP_ELEM_TEC1            =11, // Temperature element TEC1 (peltier closest to sensor)
	XI_TEMP_ELEM_TEC2            =12, // Temperature element TEC2 (peltier)
	XI_TEMP_ELEM_FAN1            =31, // Temperature element fan current or rotation
...
typedef enum
{
	...
	XI_CMS_INTENT_RELATIVE_COLORIMETRIC=1, // CMS intent relative colorimetric
	XI_CMS_INTENT_SATURATION     =2, // CMS intent saturation
	XI_CMS_INTENT_ABSOLUTE_COLORIMETRIC=3, // CMS intent absolute colorimetric
...
typedef enum
{
	...
	XI_SENSOR_FEATURE_ACQUISITION_RUNNING=3, // Sensor acquisition is running status. Could be stopped by setting of 0.
...
{			
	...
	XI_GenTL_Image_Format_BayerRG14                    =0x8000010C, // Pixel format one 14 bit pixel in 2 bytes	
	XI_GenTL_Image_Format_BayerGR14                    =0x8000020C, // Pixel format one 14 bit pixel in 2 bytes	
	XI_GenTL_Image_Format_BayerBG14                    =0x8000030C, // Pixel format one 14 bit pixel in 2 bytes	
	XI_GenTL_Image_Format_BayerGB14                    =0x8000040C, // Pixel format one 14 bit pixel in 2 bytes	
	...
	XI_GenTL_Image_Format_BayerBG14p                   =0x8100010E, // Pixel format 14bit packed (4 pixels in 7 bytes)	
	XI_GenTL_Image_Format_BayerGB14p                   =0x8100020E, // Pixel format 14bit packed (4 pixels in 7 bytes)	
	XI_GenTL_Image_Format_BayerGR14p                   =0x8100030E, // Pixel format 14bit packed (4 pixels in 7 bytes)	
	XI_GenTL_Image_Format_BayerRG14p                   =0x8100040E, // Pixel format 14bit packed (4 pixels in 7 bytes)	
	XI_GenTL_Image_Format_Mono14p                      =0x8100000E, // Pixel format 14bit packed (4 pixels in 7 bytes)	
	...
	XI_GenTL_Image_Format_xiMono12g96l_m9e3r16         =0x85000010, // Pixel format 16bit	
	...
	XI_GenTL_Image_Format_xiMono_m9e3r16               =0x84000010, // Pixel format 16bit	
	...
	XI_GenTL_Image_Format_xiMono12p_m9e3r16            =0x81000010, // Pixel format 16bit	
...
typedef struct
{
	...
	DWORD         image_user_data;// ImageUserData controlled by user application using ImageUserData or XI_PRM_IMAGE_USER_DATA parameter
	DWORD         exposure_sub_times_us[5];// Array with five sub exposures times in microseconds used by XI_TRG_SEL_MULTIPLE_EXPOSURES or hardware controlled HDR
